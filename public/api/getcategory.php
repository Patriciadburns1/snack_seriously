...
$query = "SELECT `p`.`ID`, `p`.`name`, `d`.`img_url` FROM `products` AS `p` JOIN `details` AS `d` ON `d`.`product_id` = `p`.`ID` WHERE `p`.`category_id` = 1 ORDER BY `p`.`ID` ASC LIMIT 12 OFFSET 0";
